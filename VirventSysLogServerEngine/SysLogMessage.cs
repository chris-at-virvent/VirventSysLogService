...
//        private const string winPattern = @"\<(?<pri>\d{1,3})\>(?<bsdts>((Jan)|(Feb)|(Mar)|(Apr)|(May)|(Ju[nl])|(Aug)|(Sep)|(Oct)|(Nov)|(Dec))\s\d\d\s\d\d\:\d\d\:\d\d)\s(?<hn>\S+)\s(?<an>\S+)\s(?<pr>\S+)\s(?<msgid>\S+)";
//        private const string snortPattern = @"(?<msg>\[\S+\].+?)\s(?<clas>\[.+\]{1})\s(?<pri>\[.+\]{1})\s(?<pro>\{\S+\})\s(?<src>\S+)\s..\s(?<des>\S+)";
...
//            Regex mywin = new Regex(winPattern);

//            return myre.IsMatch(msg) | mybsd.IsMatch(msg) | mywin.IsMatch(msg);
//        }
